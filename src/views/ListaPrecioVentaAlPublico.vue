<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <datasource ref="remoteDataSourceListaPrecioVentaalPublico" :transport-read="readData"
        :schema-model-fields="schemaModelFields" :page-size='100'>
      </datasource>
      <grid ref="grid" :height="'95vh'" :data-source-ref="'remoteDataSourceListaPrecioVentaalPublico'"
        :sortable-mode="'multiple'" :pageable-page-sizes="[5, 10, 15, 20, 25, 100, 250, 500]" :filterable="true"
        :reorderable="true" :resizable="true" :groupable="false" :column-menu="true" :navigatable="true"
        :toolbar="toolbarTemplate" :allow-copy="true" :selectable="false">
        <grid-column title="Lista & Art." :template="templateListaArt" :width="100" :locked="true"></grid-column>
        ...
        <grid-column field="Precio_base_pesos_contado_SI" title="Precio base contado SI" :format="'{0:c}'"
          :width="100"></grid-column>
        <grid-column field="M2xCaja" title="M2 x Caja" :width="100"></grid-column>
        <grid-column field="Fecha_impresion" title="Fecha impresión" type="date" format="{0:dd/MM/yyyy}"
          :width="100"></grid-column>
...
export default {
  ...
  data: function () {
    return {
      ...
      schemaModelFields: {
        ...
        Precio_base_pesos_contado_SI: { type: 'number' },
        M2xCaja: { type: 'string' },
  ...
  computed: {
    UrlApiBase() {
      return `${process.env.VUE_APP_API_BASE}/listapreciosventaalpublico`
    },
    options() {
      return {
  ...
  methods: {
    ...
    templateListaArt: function (item) {
      return item.ARPV_LISTA_PRECVTA + item.ARTS_ARTICULO_EMP
    },
