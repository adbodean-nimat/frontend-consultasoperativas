<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <datasource ref="remoteDataSourceNPCPEI" :transport-read="readData" :schema-model-fields="schemaModelFields"
        :page-size='100'>
      </datasource>
      <grid ref="grid" :height="'95vh'" :data-source-ref="'remoteDataSourceNPCPEI'" :sortable-mode="'multiple'"
        :pageable-page-sizes="[5, 10, 15, 20, 25, 100]" :filterable="true" :reorderable="true" :resizable="true"
        :column-menu="true" :navigatable="true" :toolbar="toolbarTemplate" :allow-copy="true" :selectable="true"
        :auto-bind="false">
        <grid-column :field="'NPCA_TIPO_NPCA'" :title="'Tipo'" :width="100"></grid-column>
        <grid-column :field="'NPCA_NUMERO_NPCA'" :title="'Número'" :width="100"></grid-column>
        <grid-column :field="'NPCA_FECHA_EMI'" :title="'Fecha emisión'" :width="150"
          template="#: kendo.toString(kendo.parseDate(NPCA_FECHA_EMI, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
        <grid-column :field="'CLIE_NOMBRE'" :title="'Cliente'"></grid-column>
        <grid-column :field="'VEND_NOMBRE'" :title="'Vendedor'"></grid-column>
        <grid-column :field="'ControlCrediticio'" :title="'Inconveniente'"></grid-column>
      </grid>
    </div>
  </div>
</template>
  ...
  data: function () {
    return {
      ...
      schemaModelFields: {
        ...
        VEND_NOMBRE: { type: 'string' },
        ControlCrediticio: { type: 'string' }
      }
    }
  },
  computed: {
    UrlApiBase() {
      return `${process.env.VUE_APP_API_BASE}/npconproblemaei/`
    },
    token() {
