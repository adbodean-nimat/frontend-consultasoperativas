<template>
    <div class="directive-fullscreen-wrapper">
      ...
      <datasource ref="remoteDataSourceFamiliaArticulos"
       ...
       methods: {
            ...
            parameterMap: function(options, operation) {            
                ...
                if (operation == 'destroy') {
                    ...
                    let params = {
                    "cod_fami_art": JSON.stringify(options.models[0].cod_fami_art),
                    "nombre_fami_art": JSON.stringify(options.models[0].nombre_fami_art),
                    "nro_orden_de_la_fami": JSON.stringify(options.models[0].nro_orden_de_la_fami),
                    "set_de_la_familia": JSON.stringify(options.models[0].set_de_la_familia),
                    };
                    let json = JSON.stringify(params);
                    var destroyUrl = `${process.env.VUE_APP_API_BASE}/familiadearticulo/`
                    $.ajax({
                        method: "DELETE",
                        url: destroyUrl + Id,
                        dataType: "json",
                ...
                if (operation == 'create') {
                    ...
                    let params = JSON.stringify(options.models[0],["cod_fami_art", "nombre_fami_art", "nro_orden_de_la_fami", "set_de_la_familia"])
                    let json = JSON.parse(params)
                    var createUrl = `${process.env.VUE_APP_API_BASE}/familiadearticulo/`
                    $.ajax({
                        method: "POST",
                        url: createUrl,
                        dataType: "json",
                ...
                if (operation == 'update') {
                    ...
                    let params = JSON.stringify(options.models[0],["cod_fami_art", "nombre_fami_art", "nro_orden_de_la_fami", "set_de_la_familia"]);
                    let json = JSON.parse(params);
                    var updateUrl = `${process.env.VUE_APP_API_BASE}/familiadearticulo/`
                    $.ajax({
                        method: "PUT",
                        url: updateUrl + Id,
                        dataType: "json",
