<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <component id="IdpdfTemplate" :is="'script'" type="text/x-kendo-template">
        ...
        <div class="pdfFont container position-absolute top-0" style="margin-top: 18px;">
          ...
          <div class="row">
            ...
            <div class="col-sm text-end"><small><span id="cod_cte"></span> <span id="dvc1listaprecvta"></span></small>
            </div>
            <div class="col-sm text-end"><small><span id="idemArtsEmp"></span></small></div>
            <div class="col-sm text-end" style="margin-right: 50px;"><strong>VA RV</strong></div>
          </div>
          <div class="row">
            <div class="col-sm"><strong>
                <h1>Productos de ACOPIO</h1>
              </strong></div>
          </div>
          <div class="row">
            <div class="col">
              <p>Lista provisoria. La lista definitiva queda confirmada cuando se formaliza el acopio, con la recepción
                del pago total y la facturación.<br>
      ...
      <datasource ref="remoteDataSourceRubrosVtasAcopio" :transport-read="readData"
        :schema-model-fields="schemaModelFields" :group="groupDefinition" :page-size='100'>
      </datasource>
      <grid ref="grid" :height="'95vh'" :data="'remoteDataSourceRubrosVtasAcopio'"
        :data-source-ref="'remoteDataSourceRubrosVtasAcopio'" :pageable-page-sizes="[100, 200, 400, 'all']"
        :sortable-mode="'multiple'" :groupable-enabled="false" :filterable="true" :filterable-extra="false"
        :reorderable="true" :resizable="true" :column-menu="true" :navigatable="false" :toolbar="toolbarTemplate"
        :allow-copy="true" :selectable="true" :auto-bind="false" :excel-file-name="'LP VN - Rubros Vtas Acopio.xlsx'"
        :excel-all-pages="true" :excel-filterable="false" :pdf-file-name="'LP VN - Rubros Vtas Acopio.pdf'"
        :pdf-all-pages="true" :pdf-avoid-links="false" :pdf-paper-size="'A4'" :pdf-margin-top="200"
        :pdf-margin-bottom="10" :pdf-margin-left="10" :pdf-margin-right="10" :pdf-landscape="false"
        :pdf-repeat-headers="false" :pdf-scale="0.5" :pdf-template="pdfTemplate" @excelexport="excelExport"
        @pdfexport="pdfExport" @databound="dataBound">
        ...
        <grid-column field="ARTS_UNIMED_STOCK" title="&nbsp;" :column-menu="false" :width="40"
          :hidden="false"></grid-column>
        <grid-column field="PRECIO_LISTA_CON_IVA" title="Precio lista c/IVA"
          template="#: kendo.toString(PRECIO_LISTA_CON_IVA, 'c2')#" :hidden="false" :width="100"></grid-column>
        <grid-column field="DCA1_POR_DESCUENTO" title="Dto. Cliente" :width="80"
          template="#: DCA1_POR_DESCUENTO == null ? '' : DCA1_POR_DESCUENTO + '%' #" :hidden="false"></grid-column>
        <grid-column field="dtoMonto" title="Dto. Monto" :template="dtoMonto" :width="80" :hidden="false"></grid-column>
        <grid-column field="dtoFinan" title="Dto. Financ." :template="dtoFinan" :width="80"
          :hidden="false"></grid-column>
        <grid-column field="precioContado" title="Precio contado c/IVA c/Dtos" :template="precioContado" :hidden="false"
          :width="100"></grid-column>
        <grid-column field="Modif" title="Modif" :hidden="false" :width="50" :filterable-multi="true"></grid-column>

        <grid-column field="VeriModif" title="Verfi Modif" :template="VeriModif" :hidden="true"
          :width="50"></grid-column>
        <grid-column field="SinModif" title="Sin Modif" :template="SinModif" :hidden="true" :width="50"></grid-column>
        <grid-column field="VeriSinModif" title="Verfi sin Modif" :template="VeriSinModif" :hidden="true"
          :width="50"></grid-column>
        <grid-column field="ArtsArticuloEmp" title="Código" :template="ArtsArticuloEmp" :column-menu="false"
          :hidden="true"></grid-column>
        <grid-column field="ARVE_RUBRO_VENTA" :hidden="true"></grid-column>
        <grid-column field="RUBV_NOMBRE" :hidden="true"
          :group-header-template="groupHeaderTemplateFamiliaArt"></grid-column>
        <grid-column field="templateRUBV_NOMBRE" :template="templateRUBV_NOMBRE" :hidden="true"></grid-column>
        ...
        <grid-column field="dvc1listaprecvta" :template="dvc1listaprecvta" :hidden="true"></grid-column>

        <grid-column field="DVC1_LISTA_PRECIO_ACOPIO" :hidden="true" :width="50"></grid-column>
        ...
        <grid-column field="Fecha_cambio_precios_hasta"
          ...
          :hidden="true" :width="80"></grid-column>
...
export default {
  ...
  data: function () {
    return {
      ...
      teleport: true,
      pageOnly: true,
      title: 'Lista de precios - Rubros Ventas Acopio',
      dataSource: ['remoteDataSourceRubrosVtasAcopio'],
      schemaModelFields: {
        ...
        ARVE_BLOQUEO_VENTA: { type: 'string' },
  ...
  methods: {
    ...
    pdfExport: function () {
      ...
      var idArts = document.getElementById("idemArtsEmp");
      var idemArts = idArts.innerText = idemArtsArticuloEmp.length + ' Art'
    },
    pdfTemplate: function (e) {
      var templateItem = kendo.template(kendo.jQuery('#IdpdfTemplate').html())
    ...
    dtoFinan: function () {
      ...
      return kendo.toString(DtoFinan + '%')
    },
    dtoMonto: function () {
      var grid = this.$refs.grid.kendoWidget();
      var gridElement = grid.element;
      var numericDtoMonto = gridElement.find('#dtoxmonto');
      var DtoMonto = numericDtoMonto.data("kendoNumericTextBox").value();
      return kendo.toString(DtoMonto + '%')
    },
    precioContado: function (item) {
      ...
      var cal4 = (1 - DtoMonto / 100)
      var precioTotal = cal * cal4 * (iDCA1_POR_DESCUENTO ? cal2 : agregaruna) * cal3 * (iCOTI_COTIZACION ? iCOTI_COTIZACION : agregaruna);
    ...
    dvc1listaprecvta: function (item) {
      var templateDvc1 = '<span id="dvc1listaprevcta">' + item.DVC1_LISTA_PRECIO_ACOPIO + '</sapn>'
      return templateDvc1
    },
  ...
  mounted: function () {
    ...
    toolbarElement.on("click", ".filter-clear", function () {
      ...
      numericDtoxMonto.data("kendoNumericTextBox").value(null);
