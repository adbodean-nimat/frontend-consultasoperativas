    ...
    <div class="container-fluid page-grid">
      <div class="encabezado-titulo">
        <div style="margin-left: 5px; color: white;" class="icon-title">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#fff" class="bi bi-card-text"
            viewBox="0 0 16 16">
            <path
      ...
      <datasource ref="remoteDataSourceCartelM2Saldo" :transport-read="readData"
        :schema-model-fields="schemaModelFields" :transport-parameter-map="parameterMap"
        :aggregate="aggregateDefinition" :page-size='1000'>
      </datasource>
      <grid ref="grid" :height="'95vh'" :data-source-ref="'remoteDataSourceCartelM2Saldo'" :sortable-mode="'multiple'"
        :sortable-allow-unsort="true" :sortable-show-indexes="true" :pageable-always-visible="false" :filterable="true"
        ...
        :scrollable-virtual="false" :scrollable-endless="false" @databound="dataBound">
        ...
        <grid-column field="Clas_Art_2" title="Clasif. Art. 2"></grid-column>
        <grid-column field="M2xCaja" title="M2 x Caja"></grid-column>
        <grid-column field="Pre_M2_UN_Lista_con_IVA_L1" title="Precio M2/UN Lista c/IVA"
          template="#: kendo.toString(Pre_M2_UN_Lista_con_IVA_L1, 'c2')#"></grid-column>
        <grid-column field="Pre_M2_UN_Cdo_con_IVA_L1" title="Precio M2/UN Cdo. c/IVA"
          template="#: kendo.toString(Pre_M2_UN_Cdo_con_IVA_L1, 'c2')#"></grid-column>
        <grid-column field="Pre_M2_UN_Cdo_con_IVA_L1_Redond" title="Precio M2/UN Cdo. c/IVA Redondeo"
          template="#: kendo.toString(Pre_M2_UN_Cdo_con_IVA_L1_Redond, 'c0')#"></grid-column>
        <grid-column title="Precio M2/UN Cdo. con Dto. Saldo" :template="PrecioDtoSaldo"></grid-column>
      </grid>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    UrlApiBase() {
      return `${process.env.VUE_APP_API_BASE}/m2saldo/`
    },
    options() {
      return {
  ...
  methods: {
    ...
    toolbarTemplate: function () {
      var templateHtml =
        ...
        '<input class="is-valid" id="codart" type="number" required/>' +
        '<div class="invalid-feedback">Falta completa este campo.</div>' +
        '</div>' +
        '</div>' +
        '<div class="col">' +
        '<label class="col-form-label" style="margin-right:5px">Descuento:</label>' +
        '<div id="input" class="col d-flex flex-column">' +
        '<input class="is-valid" id="dtofinan" type="number" required/>' +
        '<div class="invalid-feedback">Falta completa este campo.</div>' +
        '</div>' +
        '</div>' +
        '<div class="col">' +
        '<label class="col-form-label" style="margin-right:5px">Partida Origen:</label>' +
        '<div id="input" class="col d-flex flex-column">' +
        '<input class="is-valid" id="partorigen" type="text" required/>' +
        '<div class="invalid-feedback">Falta completa este campo.</div>' +
        '</div>' +
        '</div>' +
    ...
    PrecioDtoSaldo: function (item) {
      var grid = this.$refs.grid.kendoWidget();
      var gridElement = grid.element;
      var PorcDto = gridElement.find('#dtofinan')
      var DtoFinan = PorcDto.data("kendoNumericTextBox").value();
      var PreCdoCIVA = item.Pre_M2_UN_Cdo_con_IVA_L1
      var PreCDto = PreCdoCIVA * (1 - DtoFinan / 100)
      return kendo.toString(PreCDto, "c2")
    },
  },
  mounted: function () {
    var grid = this.$refs.grid.kendoWidget();
    ...
    var numericPartOrigen = gridElement.find('#partorigen');

    numericPartOrigen.kendoTextBox({})
    ...
    toolbarElement.on("click", ".print", function () {
      ...
      var PorcDto = numericDtoFinan.data("kendoNumericTextBox").value();
      var PartOrigen = numericPartOrigen.data("kendoTextBox").value();
      var itemPrecioLista = data[0].Pre_M2_UN_Lista_con_IVA_L1
      var itemPrecioCdo = data[0].Pre_M2_UN_Cdo_con_IVA_L1
      var itemPrecioCdoRed = data[0].Pre_M2_UN_Cdo_con_IVA_L1_Redond
      var itemPrecioCdoSaldo = itemPrecioCdo * (1 - PorcDto / 100)
      var itemM2xCaja = data[0].M2xCaja
      var theDate = new Date();
      var itemTextoPresentacion = "Venta de saldo con el " + PorcDto + "% de descuento";
      var itemPartidaOrigen = "Partida Origen: " + PartOrigen;
      var itemTextPresentacionxCaja = "Venta por caja cerrada. Cada caja rinde " + itemM2xCaja + "m2 aproximadamente."
      ...
      var contentBody =
        ...
        '<span><small>' + itemPartidaOrigen + '</small></span>' +
        '</div>' +
        '</div>' +

        '<div class="row pb-2 mb-4 border-bottom border-success border-3">' +
        '<div class="col text-start">' +
        '<span><small>Precio del M2: </small></span><span style="font-size: 20px;"><strong>' + kendo.toString(itemPrecioCdo, "c2") + '</strong></span>' +
        '</div>' +
        ...
        '<div class="row">' +
        '<div class="col text-center">' +
        '<span style="font-size:200px; font-weight: bold;">' + kendo.toString(itemPrecioCdoSaldo, "c0") + '</span>' +
        '</div>' +
        '</div>' +
        ...
        '<div class="row mb-4">' +
        '<div class="col text-center"><span><strong>' + itemTextPresentacionxCaja + '</strong></span></div>' +
        '</div>' +
      ...
      var contentBodySinOrigen =
        ...
        '<div class="row">' +
        '<div class="col text-center">' +
        '<span style="font-size:10rem; font-weight: bold;">' + kendo.toString(itemPrecioCdoSaldo, "c0") + '</span>' +
        '</div>' +
        '</div>' +
      ...
      if (PartOrigen == "") {
        printableContent = contentBodySinOrigen
      } else {
