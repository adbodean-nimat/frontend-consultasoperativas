<template>
    <div class="directive-fullscreen-wrapper">
      <div class="container-fluid page-grid">
        ...
        <datasource ref="remoteDataRowaPlanCanje"
        ...
        <grid ref="grid"
              :height="'95vh'"
              :data-source-ref="'remoteDataRowaPlanCanje'"
              ...
              :selectable="true"
              >
              <grid-column :field="'Cliente_devuelve_mediante_NC'" :title="'Cliente devuelve mediante NC'"></grid-column>
              <grid-column :field="'Valor_de_la_NC'" :title="'Valor de la NC'" :format="'{0:c0}'"></grid-column>
              <grid-column :field="'Producto_nuevo_que_lleva_el_cliente'" :title="'Producto nuevo que lleva el cliente'"></grid-column>
              <grid-column :field="'Precio_sin_Plan_Canje_L1_Cdo_Plataforma'" :title="'Precio sin Plan Canje - L1 Cdo. Plataforma'" :format="'{0:c0}'"></grid-column>
              <grid-column :field="'Precio_con_plan_canje_que_paga_el_cliente'" :title="'Precio con plan canje que paga el cliente'" :format="'{0:c0}'"></grid-column>
              <grid-column :field="'Descuento_incluido_en_precio_con_plan_canje'" :title="'Descuento incluido en precio con plan canje'" :format="'{0:p0}'"></grid-column>
        </grid>
      </div>
    </div>
</template>
    ...
    export default {
      ...
      data: function () {
             return {
                ...
                schemaModelFields: {
                  ...
                  Producto_nuevo_que_lleva_el_cliente: {type: 'string'},
                  Precio_sin_Plan_Canje_L1_Cdo_Plataforma: {type: 'number'},
                  Precio_con_plan_canje_que_paga_el_cliente: {type: 'number'},
                  Descuento_incluido_en_precio_con_plan_canje: {type: 'number'}
                }
             }
      },
      computed: {
        UrlApiBase(){
          return `${process.env.VUE_APP_API_BASE}/rowaplancanje`
        },
        options () {
          return {
      ...
      methods: {
        readData: function (e) {
              ...
              var urlApi = `${process.env.VUE_APP_API_BASE}/rowaplancanje`
              $.ajax({
                url: urlApi,
