<template>
    <div class="directive-fullscreen-wrapper">
      <div class="container-fluid page-grid">
        ...
        <datasource ref="remoteDataSourceCombo"
        ...
        <grid ref="grid"
              :height="'95vh'"
              :data-source-ref="'remoteDataSourceCombo'"
              :sortable-mode="'multiple'"
              :sortable-allow-unsort="true"
              :sortable-show-indexes="true"
              :pageable-page-sizes="[50, 100, 200, 500]"
              :filterable="true"
              :reorderable="true"
              :resizable="true"
              :groupable="true"
              ...
              <grid-column field="StockUni" title="Stock Uni." :aggregates="['sum']" group-footer-template="#: sum == null ? '' : sum  #"></grid-column>
              <grid-column field="Uni_Pte_Entr_NP" title="Uni. Pte. Entrega NP" :aggregates="['sum']" group-footer-template="#: sum == null ? '' : sum  #"></grid-column>
              <grid-column field="Stock_Dispon" title="Stock Disponible" :aggregates="['sum']" group-footer-template="#: sum == null ? '' : sum  #"></grid-column>
              <grid-column field="Pre_Nomal_L1_Cdo" title="Precio Normal L1 Cdo." template="#: kendo.toString(Pre_Nomal_L1_Cdo, 'c2')#" :aggregates="['sum']" group-footer-template="#: kendo.toString(sum, 'c2')#"></grid-column>
              <grid-column field="Pre_Oferta_Cdo_x_Uni" title="Precio Oferta Cdo. por Uni."  template="#: kendo.toString(Pre_Oferta_Cdo_x_Uni, 'c2')#" :aggregates="['sum']" group-footer-template="#: kendo.toString(sum, 'c2')#"></grid-column>
              <grid-column field="Desarmable" title="¿Desarmable?"></grid-column>
              <grid-column field="Vigente_desde" title="Fecha vigente desde" template="#: Vigente_desde == null ? '' : kendo.toString(kendo.parseDate(Vigente_desde, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
              <grid-column field="CMBA_FECHA_VIG_HASTA" title="Fecha vigente hasta" template="#: kendo.toString(kendo.parseDate(CMBA_FECHA_VIG_HASTA, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
              <grid-column field="Porcent_Dto_incluido_en_oferta" title="Dto. Incluido en Oferta" template="#: kendo.format('{0:p2}', Porcent_Dto_incluido_en_oferta / 100)#"></grid-column>
