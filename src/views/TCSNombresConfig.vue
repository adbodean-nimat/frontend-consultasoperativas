<template>
    <div class="directive-fullscreen-wrapper">
      ...
      <datasource ref="remoteDataConstSecoNombresConfig"
      ...
      <grid ref="grid"
                  :height="'100vh'"
                  :data-source-ref="'remoteDataConstSecoNombresConfig'"
                  :navigatable="true"
                  :pageable='false'
                  :editable="'inline'"
                  :toolbar="['create']">
            <grid-column :field="'id'" :hidden="true"></grid-column>
            <grid-column :field="'cod_conf_cs'" :title="'Código Config. CS'"></grid-column>
            <grid-column :field="'nombre_conf_cs'" :title="'Nombre Config. CS'" ></grid-column>
    ...
    export default {
      ...
      data () {
             return {
                ...
                fields: {
                    ...
                    cod_conf_cs: {type: 'string', validation:{
                      required: true,
                      validarNumeric: function(input) { 
                        if (input.is("[name='cod_conf_cs']") && input.val() != "") {
                            input.attr("data-validarNumeric-msg", "El campo debe ser numérico.");
      ...
      computed: {
        UrlApiBase(){
              return `${process.env.VUE_APP_API_BASE}/constseconombresconfig/`
        },
        options () {
          return {
       ...
       methods: {
            ...
            parameterMap: function(options, operation) {            
                ...
                if (operation == 'destroy') {
                    ...
                    let params = {
                    "cod_conf_cs": JSON.stringify(options.models[0].cod_conf_cs),
                    "nombre_conf_cs": JSON.stringify(options.models[0].nombre_conf_cs),
                    };
                    let json = JSON.stringify(params);
                    var destroyUrl = `${process.env.VUE_APP_API_BASE}/constseconombresconfig/`
                    $.ajax({
                        method: "DELETE",
                        url: destroyUrl + Id,
                        dataType: "json",
                ...
                if (operation == 'create') {
                    let params = JSON.stringify(options.models[0],["cod_conf_cs", "nombre_conf_cs"])
                    let json = JSON.parse(params)
                    var createUrl = `${process.env.VUE_APP_API_BASE}/constseconombresconfig/`
                    $.ajax({
                        method: "POST",
                        url: createUrl,
                        dataType: "json",
                ...
                if (operation == 'update') {
                    ...
                    let params = JSON.stringify(options.models[0],["cod_conf_cs", "nombre_conf_cs"]);
                    let json = JSON.parse(params);
                    var updateUrl = `${process.env.VUE_APP_API_BASE}/constseconombresconfig/`
                    $.ajax({
                        method: "PUT",
                        url: updateUrl + Id,
                        dataType: "json",
