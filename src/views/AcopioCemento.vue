<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <datasource ref="remoteDataSourceAcopioCementoLN" :transport-read="readData"
        :schema-model-fields="schemaModelFields" :page-size='100' :aggregate="aggregateDefinition">
      </datasource>
      <grid ref="grid" :height="'95vh'" :data-source-ref="'remoteDataSourceAcopioCementoLN'" :sortable-mode="'multiple'"
        ...
        :auto-bind="false" @databound="dataBound">
        <grid-column :field="'CVCL_FECHA_EMI'" :title="'Fecha'"
          template="#: kendo.toString(kendo.parseDate(CVCL_FECHA_EMI, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
        <grid-column :field="'Dia_semana'" :title="'Día semana'"></grid-column>
        <grid-column :field="'Cantidad_de_facturas'" :title="'Facturado'" :aggregates="['sum']"
          :footer-template="'#=sum#'"></grid-column>
        <grid-column :field="'Cantidad_de_remitos'" :title="'Remitos'" :aggregates="['sum']"
          :footer-template="'#=sum#'"></grid-column>
        <grid-column :field="'SumaDePendentregNP'" :title="'Pend. Entrega NP'" :hidden="true"></grid-column>
        <grid-column :field="'Stock_Cemento_todos_los_Depos'" :title="'Stock Cemento todos los Depositos'"
