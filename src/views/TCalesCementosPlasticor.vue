<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <datasource ref="remoteDataSourceCalesCementosPlasticor" :transport-read="readData" :transport-update="updateData"
      ...
      <grid ref="grid" :height="'100vh'" :data-source-ref="'remoteDataSourceCalesCementosPlasticor'" :navigatable="true"
        :filterable="true" :pageable='false' :editable="'inline'" :toolbar="['create']">
        <grid-column :field="'id'" :title="'Id'" :hidden="true"></grid-column>
        <grid-column :field="'cod_articulos'" :title="'Código Art.'"></grid-column>
        <grid-column :field="'nombre_articulos'" :title="'Nombre Art.'"></grid-column>
...
export default {
  name: 'ArtCalesCementosPlasticor',
  directives: {
    fullscreen,
  },
  components: {
  ...
  data() {
    return {
      ...
      fields: {
        ...
        cod_articulos: { type: 'string' },
        nombre_articulos: { type: 'string' }
      }
    }
  },
  computed: {
    UrlApiBase() {
      return `${process.env.VUE_APP_API_BASE}/calescementosplasticor/`
    },
    token() {
  ...
  methods: {
    ...
    updateData: function (e) {
      ...
      var data = kendo.stringify(e.data.models[0], ["cod_articulos", "nombre_articulos"])
      kendo.jQuery.ajax({
        method: 'PUT',
        type: 'PUT',
        url: urlApi + id,
