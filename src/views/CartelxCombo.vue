<template>
  <div class="directive-fullscreen-wrapper">
    <div class="container-fluid page-grid">
      ...
      <datasource ref="remoteDataSourceCartelCombo" :transport-read="readData" :schema-model-fields="schemaModelFields"
        :transport-parameter-map="parameterMap" :aggregate="aggregateDefinition" :page-size='1000'>
      </datasource>
      <grid ref="grid" :height="'95vh'" :data-source-ref="'remoteDataSourceCartelCombo'" :sortable-mode="'multiple'"
        ...
        <grid-column field="CMBD_POR_DESC" title="Descuento"
          template="#: kendo.format('{0:p2}', CMBD_POR_DESC / 100)#"></grid-column>
        <grid-column field="CMBA_FECHA_VIG_DESDE" title="Fecha vigente desde"
          template="#: kendo.toString(kendo.parseDate(CMBA_FECHA_VIG_DESDE, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
        <grid-column field="Fecha_vigencia_hasta" title="Fecha vigente hasta"
          template="#: kendo.toString(kendo.parseDate(Fecha_vigencia_hasta, 'yyyy-MM-dd'), 'dd/MM/yyyy') #"></grid-column>
        <grid-column field="Clas_Art_2" title="Clasif. Art. 2" :hidden="true"></grid-column>
        <grid-column field="Pre_M2_UN_Lista_con_IVA_L1" title="Precio Lista"
          template="#: kendo.toString(Pre_M2_UN_Lista_con_IVA_L1, 'c2')#" :aggregates="['sum']"
          footer-template="#: kendo.toString(sum, 'c2')#"></grid-column>
        <grid-column field="Pre_M2_UN_Cdo_con_IVA_L1" title="Precio Cdo."
          template="#: kendo.toString(Pre_M2_UN_Cdo_con_IVA_L1, 'c2')#" :aggregates="['sum']"
          footer-template="#: kendo.toString(sum, 'c2')#"></grid-column>
        <grid-column title="Precio Cdo. Subtotal" :template="templatePrecioCdoSubtotal" :hidden="true"></grid-column>
        <grid-column field="Pre_M2_UN_Cdo_con_IVA_L1_Redond_con_dto_combo" title="Precio Cdo. c/Dto."
          template="#: kendo.toString(Pre_M2_UN_Cdo_con_IVA_L1_Redond_con_dto_combo, 'c2')#" :aggregates="['sum']"
          footer-template="#: kendo.toString(sum, 'c2')#"></grid-column>
        <grid-column title="Precio Cdo. c/Dto. subtotal" :template="templatePrecioCdoTotal"
  ...
  mounted: function () {
    ...
    toolbarElement.on("click", ".print", function () {
      ...
      var contentBodyConCuotas =
        ...
        '<div class="row">' +
        '<div class="col text-center">' +
        '<p class="uppercase text-decoration-underline"><strong>PAGALO EN ' + NroCuotas + ' CUOTAS SIN INTERÉS CON TARJETA DE CREDITO - VISA - MASTERCARD</strong></p>' +
