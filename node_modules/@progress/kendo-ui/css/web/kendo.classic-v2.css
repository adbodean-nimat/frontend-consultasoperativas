...
.k-calendar .k-range-split-start::after {
  left: -5px;
  right: auto;
  background-image: linear-gradient(to left, rgba(243, 88, 0, 0.25), transparent 100%);
}
.k-calendar .k-range-split-end::after {
  right: -5px;
  left: auto;
...
.k-mediaplayer-titlebar {
  color: #ffffff;
  background-image: linear-gradient(rgba(39, 39, 39, 0.7), rgba(39, 39, 39, 0));
